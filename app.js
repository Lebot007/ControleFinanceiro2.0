...
class UIManager {
    ...
    atualizarStats() {
        ...
        // Calcular cartões com uso alto
        let cartoesEmAlerta = 0;
        this.fm.cartoes.forEach(cartao => {
            ...
            if (percentualUso >= 75) cartoesEmAlerta++;
        });
        ...
        const statCartoes = document.getElementById('statCartoes');
        statCartoes.textContent = this.fm.cartoes.length;
        if (cartoesEmAlerta > 0) {
            statCartoes.innerHTML = `<span title="Cartões com mais de 75% de uso">${this.fm.cartoes.length} ⚠️</span>`;
        }
    }
    ...
    renderCartoes() {
        ...
        container.innerHTML = this.fm.cartoes.map(cartao => {
            ...
            const limiteRestante = cartao.limite - totalGasto;
            
            // Status inteligente do cartão
            let statusEmoji = '✓';
            let statusCor = 'var(--success-color)';
            if (percentualUso >= 90) {
                statusEmoji = '⚠️';
                statusCor = 'var(--danger-color)';
            } else if (percentualUso >= 75) {
                statusEmoji = '⚡';
                statusCor = 'var(--warning-color)';
            }

            return `
                <div class="cartao-card" style="background: linear-gradient(135deg, ${cartao.cor} 0%, ${this.escurecer(cartao.cor)} 100%);">
                    <div class="cartao-header">
                        <div class="cartao-bandeira" title="${cartao.banda}">${cartao.banda.toUpperCase().substring(0, 4)}</div>
                        <div class="cartao-actions">
                            <button onclick="uiManager.editarCartao(${cartao.id})" style="cursor: pointer; font-size: 16px;">✏️</button>
                            <button onclick="uiManager.deletarCartaoUI(${cartao.id})" style="cursor: pointer; font-size: 16px;">🗑️</button>
                        </div>
                    </div>
                    <div class="cartao-info">
                        <div class="cartao-nome">${this.escaparHtml(cartao.nome)}</div>
                        <div class="cartao-numero" style="margin-bottom: 8px;">•••• •••• •••• ••••</div>
                        <div style="font-size: 11px; opacity: 0.85; margin-bottom: 8px;">
                            <div style="margin-bottom: 4px;">Disponível: ${this.formatarMoeda(limiteRestante)}</div>
                            <div style="background: rgba(255, 255, 255, 0.2); height: 4px; border-radius: 2px; overflow: hidden;">
                                <div style="background: rgba(255, 255, 255, 0.8); height: 100%; width: ${Math.min(100, percentualUso)}%; transition: width 0.3s;"></div>
                            </div>
                        </div>
                    </div>
                    <div class="cartao-footer">
                        <div class="cartao-vencimento">Vence dia ${cartao.vencimento}</div>
                        <div class="cartao-limite">
                            <span class="cartao-limite-label" style="color: ${statusCor};">● ${statusEmoji} ${percentualUso}%</span>
            ...
            cartaoDespesasList.innerHTML = this.fm.cartoes.map(cartao => {
                ...
                return `
                    <div style="margin-bottom: 24px; padding: 16px; background: var(--bg-secondary); border-radius: 8px;">
                        <h4 style="margin-bottom: 8px;">${this.escaparHtml(cartao.nome)}</h4>
                        <p style="font-size: 12px; color: var(--text-secondary); margin-bottom: 12px;">
                            Total gasto: <strong>${this.formatarMoeda(total)}</strong> (${(total / cartao.limite * 100).toFixed(0)}% do limite)
                        </p>
                        <div style="max-height: 200px; overflow-y: auto;">
                            ${despesas.map(d => {
                                ...
                                return `
                                    <div style="font-size: 13px; padding: 8px; background: var(--bg-primary); border-radius: 4px; margin-bottom: 4px; display: flex; justify-content: space-between; align-items: center;">
                                        <span>${cat?.emoji} ${cat?.nome}</span>
                                        <span style="font-size: 11px; color: var(--text-secondary);">${data}</span>
                                        <strong style="color: var(--despesa-color);">${this.formatarMoeda(d.valor)}</strong>
                                    </div>
                                `;
                            }).join('')}
                        </div>
